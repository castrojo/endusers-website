...
test.describe('visual layout verification', () => {
  ...
  test('members-grid uses multi-column grid layout', async ({ page }) => {
    const membersGrid = await page.evaluate(() => {
      const grid = document.querySelector('.members-grid, #members-grid');
      if (!grid) return null;
      const cs = getComputedStyle(grid);
      return { display: cs.display, cols: cs.gridTemplateColumns };
    });
    expect(membersGrid).not.toBeNull();
    expect(membersGrid!.display).toBe('grid');
    // members-grid should have 2+ columns (auto-fill, minmax(280px, 1fr))
    const colCount = membersGrid!.cols.split(' ').length;
    expect(colCount).toBeGreaterThanOrEqual(2);
  });
  ...
  test('site switcher pills do not have CNCF prefix', async ({ page }) => {
    ...
    expect(pills).toContain('End Users');
